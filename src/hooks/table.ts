	...
	const query = async () => {
		...
		if (state.pageList) {
			try {
				...
				// 确保dataList始终是数组类型
				const rawData = state.isPage ? res[state.props.item] : res.data;
				state.dataList = Array.isArray(rawData) ? rawData : [];
				if (!Array.isArray(rawData)) {
					console.warn('API响应数据不是数组:', rawData);
				}
				// 处理合并
				if (state.spanMethod){
					state.spanMethod()
				...
				// 设置分页信息中的总数据条数
				state.pagination!.total = state.isPage ? res[state.props.totalCount] : 0;
			} catch (err: any) {
				// 捕获异常并显示错误提示
