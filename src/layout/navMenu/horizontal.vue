				...
				<template v-for="val in menuLists">
					...
					<template v-else>
						<el-menu-item 
							:index="val.path" 
							:key="val.path"
							@click="(val.meta.is_link && !val.meta.is_iframe) ? onLinkClick(val):null"
						>
							<template #title>
								<SvgIcon :name="val.meta.icon" />
...
const setCurrentRouterHighlight = (currentRoute: RouteToFrom) => {
	...
	if (themeConfig.value.layout === 'classic') {
		...
		if (pathSplit.length >= 4 && meta?.is_hide) state.defaultActive = pathSplit.splice(0, 3).join('/');
...
// 打开外部链接
const onLinkClick = (val: RouteItem) => {
