<template>
	<div class="layout-navbars-tagsview" :class="{ 'layout-navbars-tagsview-shadow': getThemeConfig.layout === 'classic' }">
		<el-scrollbar ref="scrollbarRef" @wheel.prevent.passive="onHandleScroll">
...
// pinia 中获取路由信息：如果是设置了固定的（is_affix），进行初始化显示
...
// 1、添加 tagsView：未设置隐藏（is_hide）也添加到在 tagsView 中（可开启多标签详情，单标签详情）
const addTagsView = (path: string, to?: RouteToFrom) => {
	...
	nextTick(async () => {
		...
		if (item?.meta?.is_link && !item.meta.is_iframe) return false;
...
// 3、关闭当前 tagsView：如果是设置了固定的（is_affix），不可以关闭
...
// 4、关闭其它 tagsView：如果是设置了固定的（is_affix），不进行关闭
...
// 5、关闭全部 tagsView：如果是设置了固定的（is_affix），不进行关闭
