...
const setIframeList = computed(() => {
	return (<RouteItems>props.list).filter((v: RouteItem) => v.meta?.is_iframeOpen);
});
// 获取 iframe 当前路由 path
	...
	nextTick(() => {
		...
		iframeRef.value.forEach((v: HTMLElement) => {
			if (v.dataset.url === val) {
				v.onload = () => {
					if (item.meta?.is_iframeOpen && item.meta.loading) item.meta.loading = false;
				};
			}
		});
	});
...
watch(
	...
	(val) => {
		...
		if (!item.meta.is_iframeOpen) item.meta.is_iframeOpen = true;
...
watch(
	...
	() => {
		...
		if (!item) return false;
		if (item.meta.is_iframeOpen) item.meta.is_iframeOpen = false;
		setTimeout(() => {
			item.meta.is_iframeOpen = true;
