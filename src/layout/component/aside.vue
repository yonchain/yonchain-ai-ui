...
const setFilterRoutes = () => {
	...
	if (themeConfig.value.layout === 'classic') {
		...
		const parentRoute = routesList.value.find(route => route.children?.some(child => child.path === currentRoute.path));
		state.menuList = parentRoute ? filterRoutesFun(parentRoute.children || []) : filterRoutesFun(routesList.value);
	} else {
...
const filterRoutesFun = <T extends RouteItem>(arr: T[]): T[] => {
	return arr
		.filter((item: T) => !item.meta?.is_hide)
		.map((item: T) => {
