...
import { baseRoutes, notFoundAndNoPower, dynamicRoutes, staticRoutes } from '/@/router/route';
...
export async function initBackEndControlRoutes() {
	...
	// 避免重复加载
	if (window.routeInitialized) return;
 ...
 * @description 用于 tagsView、菜单搜索中：未过滤隐藏的(is_hide)
 */
...
export function backEndComponent(routes: any) {
	...
	return routes.map((item: any) => {
		// 查找前端是否已定义相同路径的路由
		const frontendRoute = [...staticRoutes, ...dynamicRoutes, ...baseRoutes[0].children]
			.find(r => r.path === item.path);
		
		// 保留前端路由的关键meta配置
		if (frontendRoute?.meta) {
			item.meta = {
				...item.meta, // 后端meta
				isAuth: frontendRoute.meta.isAuth, // 保留前端认证设置
				is_hide: frontendRoute.meta.is_hide, // 保留前端隐藏设置
				...(frontendRoute.meta.skipAuth && { skipAuth: true }), // 保留跳过认证设置
				...(frontendRoute.meta.noCheckPermission && { noCheckPermission: true }) // 保留权限检查设置
			};
		}
