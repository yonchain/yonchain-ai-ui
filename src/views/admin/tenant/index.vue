		...
		<div class="layout-padding-auto layout-padding-view">
			<el-row v-show="showSearch" class="search-card">
				<div class="card-container">
					<el-form ref="queryRef" :inline="true" :model="queryForm" @keyup.enter="initData">
						<el-form-item :label="$t('tenant.name')" prop="name" class="search-input">
							<el-input
								v-model="queryForm.name"
								:placeholder="$t('tenant.inputTenantNameTip')"
								clearable
			...
			<ul class="app-list">
				<li v-for="item in appList">
					...
					<div class="mt-[12px] text-[#677182]">{{ item.created_at || '-' }}</div>
					<div class="mt-[8px] flex justify-end pt-[12px]" style="border-top: 1px solid #f0f2f5">
						<el-button type="primary" size="small" icon="Edit" @click="appDialogRef.openDialog(item.id)">{{ $t('common.editBtn') }}</el-button>
...
import { delObj, pageList } from '/@/api/admin/tenant';
...
const initData = () => {
	pageList({ page: paginationData.value.current, limit: paginationData.value.size, name: queryForm.name }).then((res) => {
		...
		storesWorkplace.setWorkplacesList(res.data);
	});
};
