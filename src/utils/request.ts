import axios, { AxiosInstance, AxiosRequestConfig, AxiosResponse } from 'axios';
import { Session } from '/@/utils/storage';
import { useMessageBox, useMessage } from '/@/hooks/message';
import qs from 'qs';
import other from './other';
import { refreshTokenApi} from '/@/api/login/index';

/**
 * 创建并配置一个 Axios 实例对象
 */
const service: AxiosInstance = axios.create({
	baseURL: import.meta.env.VITE_API_URL,
	timeout: 50000, // 全局超时时间
    paramsSerializer: {
        serialize: (params: any) => {
            return qs.stringify(params, {arrayFormat: 'repeat'});
        }
    }
});

/**
 * Axios请求拦截器，对请求进行处理
 * 1. 序列化get请求参数
 * 2. 统一增加Authorization和TENANT-ID请求头
 * 3. 自动适配单体、微服务架构不同的URL
 * @param config AxiosRequestConfig对象，包含请求配置信息
 */
service.interceptors.request.use(
	(config: AxiosRequestConfig) => {
		// 统一增加Authorization请求头, skipToken 跳过增加token
		const token = Session.getToken();
		if (token && !config.headers?.skipToken) {
			config.headers![CommonHeaderEnum.AUTHORIZATION] = `Bearer ${token}`;
		}

		// 请求报文加密
		if (config.headers![CommonHeaderEnum.ENC_FLAG]) {
			const enc = other.encryption(JSON.stringify(config.data), import.meta.env.VITE_PWD_ENC_KEY);
			config.data = {
				encryption: enc,
			};
		}

		// 自动适配单体和微服务架构不同的URL
		config.url = other.adaptationUrl(config.url);

		// 处理完毕，返回config对象
		return config;
	},
	(error) => {
		// 对请求错误进行处理
		return Promise.reject(error);
	}
);

/**
 * 响应拦截器处理函数
 * @param response 响应结果
 * @returns 如果响应成功，则返回响应的data属性；否则，抛出错误或者执行其他操作
 */
const handleResponse = (response: AxiosResponse<any>) => {
	// 如果响应码为1，表示业务错误，抛出错误
//	if (response.data.code === 1) {
//		throw response.data;
	//}

	// 针对密文返回解密
	//if (response.data.encryption) {
	//	try {
	//		const originData = JSON.parse(other.decryption(response.data.encryption, import.meta.env.VITE_PWD_ENC_KEY));
	//		response.data = originData;
	//	} catch (error) {
	//		console.error('Response decryption failed:', error);
	//		throw new Error('Response decryption failed');
	//	}
	//}

	return response.data;
};
/**
 * 添加 Axios 的响应拦截器，用于全局响应结果处理
 */
service.interceptors.response.use(handleResponse, (error) => {
	// 确保错误响应存在
	if (!error.response || !error.response.data) {
		useMessage().error('网络请求失败，请检查您的网络连接');
		return Promise.reject({ message: '网络请求失败，请检查您的网络连接' });
	}
	
	const code = error.response.data.code;
	const status = error.response.status;
	
	// 处理服务器错误（如500等）
	if (status == 500) {
		const errorMsg = '服务器内部错误,请联系管理员';
		useMessage().error(`${errorMsg}`);
		return Promise.reject({ message: `${errorMsg}`});
	}
	
	// 令牌状态过期使用刷新令牌重新获取
	if (code === 'token_expired') {
		const refreshToken = Session.get('refresh_token');
		
		// 如果没有刷新令牌，直接跳转登录页
		if (!refreshToken) {
			return handleAuthRedirect('登录已过期，请重新登录');
		}
		
		// 尝试刷新令牌
		return refreshTokenApi(refreshToken)
			.then((res) => {
				// 存储新的 token 信息
				Session.set('token', res.access_token);
				Session.set('refresh_token', res.refresh_token);
				// 重新请求
				return service.request(error.config);
			})
			.catch((err) => {
				// 根据错误类型决定是否提示
				if (err.code === 'invalid_grant') {
					return handleAuthRedirect('登录已过期，请重新登录');
				} else {
					return handleAuthRedirect(err.message || '刷新令牌失败');
				}
			});								
	}
	// 令牌无效，跳转登录页
	else if (code === 'token_invalid') {
		return handleAuthRedirect('登录无效，请重新登录');
	}
	
	// 其他错误，显示错误信息
//	const errorMsg = error.response.data.message || '请求失败';
//	useMessage().error(errorMsg);
	return Promise.reject(error.response.data);
});

/**
 * 处理认证重定向
 * @param message 提示消息
 * @returns Promise
 */
function handleAuthRedirect(message: string): Promise<never> {
	return new Promise((_, reject) => {
		useMessageBox()
			.confirm(message)
			.then(() => {
				Session.clear(); // 清除浏览器全部临时缓存
				window.location.href = '/'; // 去登录页
			})
			.catch(() => reject({ message })); // 用户取消则不跳转
	});
}

// 常用header
export enum CommonHeaderEnum {
	'TENANT_ID' = 'TENANT-ID',
	'ENC_FLAG' = 'Enc-Flag',
	'AUTHORIZATION' = 'Authorization',
}

// 导出 axios 实例
export default service;