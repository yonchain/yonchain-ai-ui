 ...
 * @param {string} item.en_name - 英文名称
...
export function setMenuI18n(item: any) {
    ...
    return i18n.global.locale.value === 'en' ? item.meta.en_name : item.meta.title;
}
...
export function handleOpenLink(val: RouteItem) {
    if (val.meta?.is_link) {
        let url = val.path;
        // 处理编码后的iframe路径
        if (url.startsWith('/iframes/')) {
            try {
                url = window.atob(url.replace('/iframes/', ''));
            } catch (e) {
                useMessage().error('URL解码失败');
                return;
            }
        }
        
        // 替换{host}占位符为当前主机名和端口
        if (url.includes('{host}')) {
            const currentHostPort = window.location.host;
            url = url.replace('{host}', currentHostPort);
        }
        
        if (verifyUrl(url)) {
            window.open(url, '_blank');
        } else {
            useMessage().error(`无效的URL: ${url}`);
        }
    }
